...
export function Inbox({ candidates, onAddCandidate, onRemoveCandidate, onRecruit, onChat, config, currencies, fame, rosterCount, deductTokens, onNotification }: InboxProps) {
  ...
  const handleScout = async () => {
    if (currencies.tokens < 1) {
      if (onNotification) onNotification("Không đủ CineTokens để sử dụng Tuyển Trạch Viên Tự Động.", 'warning');
      return;
    }
    ...
    try {
      ...
      console.error(e);
      if (onNotification) onNotification("Casting thất bại, đã xảy ra lỗi do hệ thống mạng.", 'warning');
    } finally {
      setIsScouting(false);
    }
  };
          ...
          <>
            ...
            <div className="flex-1 flex overflow-hidden">
               ...
               <div className="w-64 bg-neutral-950 p-4 overflow-y-auto thin-scrollbar shrink-0 flex flex-col gap-4">
                  ...
                  <div>
                    <h4 className="text-xs font-semibold text-neutral-500 uppercase mb-2">Hình thể</h4>
                    <BodyStatsDisplay bodyStats={activeCand.bodyStats} />
                  </div>
                  <div className="mt-auto pt-4 space-y-2 border-t border-neutral-800">
                     <button
                       onClick={() => {
                         if (!onRecruit(activeCand.id)) {
                           if (onNotification) onNotification('Không đủ Cash để ký hợp đồng.', 'warning');
                         } else {
